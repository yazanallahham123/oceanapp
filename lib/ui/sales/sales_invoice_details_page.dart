...
import '../../model/invoice/invoice_detail.dart';
import '../../model/item/item.dart';
import '../../model/transfer/transfer_master.dart';
import '../../widgets/sales_invoice_detail_widget.dart';
import '../../widgets/transfer_widget.dart';

class SalesInvoiceDetailsPage extends StatefulWidget {
  static const route = "/sales_invoice_details_page";

  final String? BLNO;

  const SalesInvoiceDetailsPage({this.BLNO});
    ...
    return BlocListener<ItemBloc, ItemState>(
        listener: (context, state) async {
          if (state is ItemInitItemState) {
            if (widget.BLNO != null)
              if (!bloc.isClosed)
                bloc.add(GetSalesInvoiceDetailsEvent(widget.BLNO!));
          } else if (state is ErrorItemState)
            pushToast(state.error);
          else if (state is GotSalesInvoiceDetailsState)
          {
            if (mounted) {
              setState(() {
                if (state.invoiceDetails.isNotEmpty) {
                  ...
                  pushToast(translate("errors.noInvoiceDetails"));
              });
            }
          }
        },
